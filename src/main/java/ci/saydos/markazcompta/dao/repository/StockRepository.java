...
public interface StockRepository extends JpaRepository<Stock, Integer>, _StockRepository {
	...
	/**
	 * Finds Stock by using idArticle as a search criteria.
	 * 
	 * @param idArticle
	 * @return An Object Stock whose idArticle is equals to the given idArticle. If
	 *         no Stock is found, this method returns null.
	 */
	@Query("select e from Stock e where e.article.id = :idArticle and e.isDeleted = :isDeleted")
	List<Stock> findByIdArticle(@Param("idArticle")Integer idArticle, @Param("isDeleted")Boolean isDeleted);
	...
	default String generateCriteria(StockDto dto, HashMap<String, java.lang.Object> param, Integer index,  Locale locale) throws Exception{
		...
		if (dto != null) {
			...
			if (dto.getIdArticle()!= null && dto.getIdArticle() > 0) {
				listOfQuery.add(CriteriaUtils.generateCriteria("idArticle", dto.getIdArticle(), "e.article.id", "Integer", dto.getIdArticleParam(), param, index, locale));
			}
			if (Utilities.notBlank(dto.getArticleLibelle())) {
				listOfQuery.add(CriteriaUtils.generateCriteria("articleLibelle", dto.getArticleLibelle(), "e.article.libelle", "String", dto.getArticleLibelleParam(), param, index, locale));
			}
