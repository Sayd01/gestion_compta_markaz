...
public class UtilisateurRoleBusiness implements IBasicBusiness<Request<UtilisateurRoleDto>, Response<UtilisateurRoleDto>> {
    ...
    public Response<UtilisateurRoleDto> create(Request<UtilisateurRoleDto> request, Locale locale) throws ParseException {
        ...
        for (UtilisateurRoleDto dto : request.getDatas()) {
            ...
            fieldsToVerify.put("userEmail", dto.getUserEmail());
            ...
            existingRole = roleRepository.findByName(dto.getRoleName(), false);
            if (existingRole == null) {
                throw new EntityNotFoundException(functionalError.DATA_NOT_EXIST("role roleName -> " + dto.getRoleName(), locale));
            }

            // Verify if utilisateur exist
            Utilisateur existingUtilisateur = null;
            existingUtilisateur = utilisateurRepository.findByEmail(dto.getUserEmail(), false);
            if (existingUtilisateur == null) {
                throw new EntityNotFoundException(functionalError.DATA_NOT_EXIST("utilisateur utilisateurEmail -> " + dto.getUserEmail(), locale));
            }

            UtilisateurRole utilisateurRole = utilisateurRoleRepository.findByUserAndRole(existingUtilisateur.getEmail(),existingRole.getName(),false);
            if (utilisateurRole != null) {
                throw new InternalErrorException(functionalError.DATA_EXIST("",locale));
            }
