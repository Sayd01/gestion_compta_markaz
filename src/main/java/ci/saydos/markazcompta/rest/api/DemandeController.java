...
public class DemandeController {
    ...
    @RequestMapping(value = "/valide", method = RequestMethod.POST, consumes = {
            "application/json"}, produces = {"application/json"})
    public Response<DemandeDto> valide(@RequestBody Request<DemandeDto> request) throws Exception {
        log.info("start method /Demande/valide");
        ...
        try {
            response = Validate.validateList(request, response, functionalError, locale);
            if (!response.isHasError()) {
                response = demandeBusiness.valide(request, locale);
            } else {
        ...
        log.info("end method /Demande/valide");
        return response;
    }
}
